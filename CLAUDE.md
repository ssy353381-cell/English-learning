...
- 詞庫 12018 字（課程單字 1107、手寫層 645、自動層其餘）。例句涵蓋率：中英對照 1944 字、只有英文示例 4135 字，其餘 5939 字只有詞義與詞形變化 —— 商務標籤 284 字與第 1 級 1175 字已全部有例句。
- 可選欄位覆蓋率：`lure` 141 字、`col` 30 字（47 組）、`rt` 45 字、`tl` 9 個文法點。純資料，補充不必動程式。
- Stage 4–6 共 30 關僅有標題，`ready: false`。

## 下一步 TODO

1. **詞庫的例句**。商務標籤 284 字與第 1 級 1175 字都已補完（手寫層 645 筆、1290 句），剩下 5939 字連一句例句都沒有，仍是詞庫最弱的一環。補的方式一樣：搬進 `lexicon-core.js` 寫兩句。剩下的量：第 2 級 766 字、第 3 級 1136 字、第 4–6 級 4037 字 —— **照級數往下做，做完一級就在 `test-logic.js` 加一項釘住**，和現有的商務字、第 1 級同一個形狀。第 2 級起有一批英式拼法（`centre`／`colour` 那種）與語氣詞，寫的時候直接說明它是哪一邊的拼法就好，不必當成另一個字。
...
5. **例句點字有 76 個字點開是空的**。掃過全部例句（兩萬字）後查不到的相異字有 76 個，三個成因分開處理：**詞庫沒收**（`online` 出現十七次最多，還有 `app`、`TV`、`cannot`、`smartphone`、`upload` —— ECDICT 那份的選字看不到這幾年的常用字，補進 `lexicon-core.js` 即可）；**`ruleForms()` 推不出來的變化**（比較級最高級 `cheaper`／`biggest`、不規則複數 `children`、不規則過去式 `meant`、`-o` 結尾的 `photos`、重複子音的 `permitted` —— 這是 `js/lexicon.js` 的規則要補，但加比較級規則會讓 `lighter` 這種本身就是名詞的字被吃掉，得先想清楚優先序）；**縮寫不還原**（`can't`、`I'm`、`I'll` —— `ExUtil.matchAny()` 早就會展開了，`Lexicon.norm()` 沒有）。專有名詞（`Taipei`、`Tokyo`、`Amy`）查不到是對的，不要收。
6. **`markup()` 吃不到重音字母**。`TOKEN_RE` 只認 `[A-Za-z]`，所以課程單字裡的 `café` 會斷成可點的 `caf` 加一個孤零零的 `é` 掉在 span 外面（`vocab-s0`／`s1b`／`s2b` 各有一處）。正規表示式加上重音範圍就好，但要順便確認 `norm()` 有沒有把 `é` 正規化成 `e`，否則點了還是查不到。

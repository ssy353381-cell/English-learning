...
**例句仍是詞庫最弱的一環**：一萬二千字裡有中英對照例句的是 2826 字，另有 4095 字只有
WordNet 的英文用法示例，其餘 5142 字目前只有詞義、音標、詞形變化與同家族。查詢卡在沒有
...
**已經補完、且由 `test-logic.js` 釘住的三批**：商務標籤（`biz`）、**第 1 級全部**與**第 2 級全部**。商務字是這個 App 的目標，第 1 級是詞頻最高的那一批，兩者查了給不出例句都等於沒收；第 2 級則是「看得懂、講不出來」的那一層 —— 讀到 `breach of contract` 知道不妙，自己寫信時卻只想得到 `break`。
往下補的順序是第 3 級（還缺 1117 字）與第 4–6 級（4025 字）。
...
- `test-logic.js` 涵蓋：`node --check`、ES5 語法、資料完整性、組題、誘答、搭配詞、最小音對、時間軸、鷹架、跳關測驗、弱點怪獸對映、SRS 範圍、`matchAny`、長難句（各段接不接得回原句、骨架夠不夠短）、三篇閱讀的跨篇題數、詞庫三層合併（含 `backfill` 補不補得回來、手寫層例句是否成對、商務字與第 1、2 級有沒有例句、一形兩主的變化形兩筆都在）。上面各節的不變量都由它釘住。
...
- 詞庫 12063 字（課程單字 1218、手寫層 1369、自動層其餘）。例句涵蓋率：中英對照 2826 字、只有英文示例 4095 字，其餘 5142 字只有詞義與詞形變化 —— 商務標籤與第 1、2 級已全部有例句。
...
1. **詞庫的例句**。商務標籤 297 字、第 1 級 1158 字與第 2 級 1737 字都已補完（手寫層 1369 筆、2852 句），剩下 5142 字連一句例句都沒有，仍是詞庫最弱的一環。補的方式一樣：搬進 `lexicon-core.js` 寫兩句。剩下的量：第 3 級 1117 字、第 4–6 級 4025 字 —— **照級數往下做，做完一級就在 `test-logic.js` 加一項釘住**（那項檢查已經寫成 `[1, 2].forEach`，補完一級就把級數加進去）。

   第 2 級補的時候學到的三件事，第 3 級照做即可：一是**英式拼法與英式用字不另收一筆**，`note` 裡說明是哪一邊、並指向對應的另一個字（`petrol`↔`gasoline`、`trousers`↔`pants`）；二是**成對的易混字要互相指過去**（`assure`／`ensure`／`insure`、`sensitive`／`sensible`、`formerly`／`formally`、`devise`／`device`）—— 這一層的字之所以「看得懂、講不出來」，多半就是因為旁邊還站著一個長得很像的字；三是**寫完要回頭掃一次 backfill 進來的 `en`**，第 2 級這 755 字裡有 38 個的 WordNet 釋義講的是另一個意思（`tackle` 是美式足球的位置、`tyre` 是黎巴嫩的港口、`tom` 直接是一句種族歧視語），碰到就自己寫一句蓋掉。

...
**例句仍是詞庫最弱的一環**：一萬二千字裡有中英對照例句的是 1539 字，另有 4135 字只有
WordNet 的英文用法示例，其餘 6344 字目前只有詞義、音標、詞形變化與同家族。查詢卡在沒有
例句時會照實說，不會假裝有。要補就是往 `lexicon-core.js` 寫，一筆兩句（日常一句、職場一句）。

**商務標籤（`biz`）那 284 字已經全部有例句**，`test-logic.js` 釘住這件事 —— 商務字是這個
App 的目標，查了給不出例句的商務字等於沒收。往下補的順序是第 1–3 級的一般字。
...
- `test-logic.js` 涵蓋：`node --check`、ES5 語法、資料完整性、組題、誘答、搭配詞、最小音對、時間軸、鷹架、跳關測驗、弱點怪獸對映、SRS 範圍、`matchAny`、詞庫三層合併（含 `backfill` 補不補得回來、手寫層例句是否成對、商務字有沒有例句）。上面各節的不變量都由它釘住。
...
- 詞庫 12018 字（課程單字 1107、手寫層 240、自動層其餘）。例句涵蓋率仍低：中英對照 1539 字、只有英文示例 4135 字，其餘 6344 字只有詞義與詞形變化 —— 但商務標籤那 284 字已全部有例句。
- 可選欄位覆蓋率：`lure` 141 字、`col` 30 字（47 組）、`rt` 45 字、`tl` 9 個文法點。純資料，補充不必動程式。
- Stage 4–6 共 30 關僅有標題，`ready: false`。

## 下一步 TODO

1. **詞庫的例句**。商務標籤的 284 字已經補完（手寫層 240 筆、480 句），剩下 6344 字連一句例句都沒有，仍是詞庫最弱的一環。補的方式一樣：搬進 `lexicon-core.js` 寫兩句，優先第 1–3 級裡出現頻率高的一般字。

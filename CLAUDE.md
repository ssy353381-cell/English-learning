...
**例句仍是詞庫最弱的一環**：一萬二千字裡有中英對照例句的是 2071 字，另有 4095 字只有
WordNet 的英文用法示例，其餘 5897 字目前只有詞義、音標、詞形變化與同家族。查詢卡在沒有
...
**已經補完、且由 `test-logic.js` 釘住的兩批**：商務標籤（`biz`）與**第 1 級全部**。商務字是這個 App 的目標，第 1 級是詞頻最高的那一批，兩者查了給不出例句都等於沒收。
往下補的順序是第 2 級（還缺 766 字）與第 3 級（1136 字）。
...
- `test-logic.js` 涵蓋：`node --check`、ES5 語法、資料完整性、組題、誘答、搭配詞、最小音對、時間軸、鷹架、跳關測驗、弱點怪獸對映、SRS 範圍、`matchAny`、長難句（各段接不接得回原句、骨架夠不夠短）、三篇閱讀的跨篇題數、詞庫三層合併（含 `backfill` 補不補得回來、手寫層例句是否成對、商務字與第 1 級有沒有例句、一形兩主的變化形兩筆都在）。上面各節的不變量都由它釘住。
...
- 詞庫 12063 字（課程單字 1218、手寫層 614、自動層其餘）。例句涵蓋率：中英對照 2071 字、只有英文示例 4095 字，其餘 5897 字只有詞義與詞形變化 —— 商務標籤與第 1 級已全部有例句。
...
1. **詞庫的例句**。商務標籤 301 字與第 1 級 1180 字都已補完（手寫層 670 筆、1340 句），剩下 5939 字連一句例句都沒有，仍是詞庫最弱的一環。補的方式一樣：搬進 `lexicon-core.js` 寫兩句。剩下的量：第 2 級 766 字、第 3 級 1136 字、第 4–6 級 4037 字 —— **照級數往下做，做完一級就在 `test-logic.js` 加一項釘住**，和現有的商務字、第 1 級同一個形狀。第 2 級起有一批英式拼法（`centre`／`colour` 那種）與語氣詞，寫的時候直接說明它是哪一邊的拼法就好，不必當成另一個字。

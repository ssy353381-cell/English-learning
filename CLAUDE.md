...
19 種：intro flashcard recall spell listen dictate speak grammar build cloze read irregular photo respond collocate phoneme convo part6 part7。
...
`part7`（多益 Part 7 雙篇閱讀，在 `part7.js`）：**兩份文件一定要同時看得見**。這一題型練的就是跨篇對照（規則在公告裡、這個人的情況在信裡），做成分頁或一次顯示一篇，練到的只剩單篇閱讀。寬畫面並排、窄畫面上下疊，但都不折疊。需要跨篇的題目掛 `both:true`，畫面標出來——那不是提示，是告訴他「只看一篇找不到是正常的」。結算另外單獨列跨篇題的答對數，整體答對率看不出有沒有真的去對照。
...
`Content.isReady(uid)` = `stage.ready` && 有 `plan` && 自己有九種內容任一（vocab／grammar／reading／photo／respond／minpair／convo／part6／part7）。九種都要算：純聽力關沒有單字也沒有文法，Part 6 關只有段落填空、雙篇閱讀關只有兩份文件，魔王關則相反 —— 題目全靠 `*UpTo()` 往前借，只有自己的短文。

`stage.ready` 只代表階段開放，關卡可分批補（沒 `plan` 就顯示「製作中」）。但解鎖是一條鏈，**可玩的關卡必須從頭連續** —— 中間空一關，後面永遠解不開。

id 前綴互斥（共用 `byId` 索引）：vocab `v` / grammar `g` / reading `r` / irregular `i` / Part 1 `p` / Part 2 `q` / 最小音對 `m` / Part 3・4 `c` / Part 6 `x` / Part 7 雙篇 `d`，各接四位數。
...
**例句仍是詞庫最弱的一環**：一萬二千字裡有中英對照例句的是 1944 字，另有 4135 字只有
WordNet 的英文用法示例，其餘 5939 字目前只有詞義、音標、詞形變化與同家族。查詢卡在沒有
...
**已經補完、且由 `test-logic.js` 釘住的兩批**：商務標籤（`biz`）284 字，以及**第 1 級全部
1175 字**。商務字是這個 App 的目標，第 1 級是詞頻最高的那一批，兩者查了給不出例句都等於沒收。
...
- `test-logic.js` 涵蓋：`node --check`、ES5 語法、資料完整性、組題、誘答、搭配詞、最小音對、時間軸、鷹架、跳關測驗、弱點怪獸對映、SRS 範圍、`matchAny`、詞庫三層合併（含 `backfill` 補不補得回來、手寫層例句是否成對、商務字與第 1 級有沒有例句）。上面各節的不變量都由它釘住。
...
- 可玩 Stage 0–4 共 55 關（1179 單字、35 文法點、75 篇短文、63 個不規則動詞、14 題 Part 1、22 題 Part 2、6 題 Part 3／4、3 篇 Part 6、2 組 Part 7 雙篇、18 組最小音對）。Stage 3、4 都已完整。
- 詞庫 12042 字（課程單字 1107、手寫層 621、自動層其餘）。例句涵蓋率：中英對照 1968 字、只有英文示例 4135 字，其餘 5939 字只有詞義與詞形變化 —— 商務標籤 301 字與第 1 級 1180 字已全部有例句。
- 例句點字：手寫例句裡小寫開頭的字已經沒有一個點開是空的（`test-logic.js` 釘住），剩下的是專有名詞。
- 可選欄位覆蓋率：`lure` 141 字、`col` 30 字（47 組）、`rt` 45 字、`tl` 9 個文法點。純資料，補充不必動程式。
- Stage 5–6 共 20 關僅有標題，`ready: false`。
...
2. **Stage 5 的 10 關**（倒裝、假設語氣進階、Part 7 三篇閱讀、長難句拆解）。三篇閱讀可以沿用 `Ex.part7` 的骨架 —— 資料的 `docs` 本來就是陣列，畫面在窄螢幕已經是上下疊，只有寬螢幕的兩欄要改成三欄。真正要新寫的是長難句拆解。
...
4. **畫面層零自動檢查**。CI 只驗語法、資料與組題，`views/*`／`exercises/*` 的 render 全靠手動，而題型模組已增至 19 種。要納入就得引入 Playwright，與零依賴衝突，先想清楚值不值得。
5. **自動層那批 WordNet 例句還有 455 個字點開是空的**。手寫的例句已經清乾淨，但自動層
   ...
   量大但價值低，排在補例句與 Stage 4 後面。
6. **同一個變化形被兩個字搶走時，先進索引的贏**。`leaves` 目前開出「葉子」而不是
   `leave`，`lives` 開出「生命」而不是 `live` —— 自動層的 `leaf`／`life` 帶著 ECDICT
   真的複數先進索引，`leave`／`live` 的規則變化在第二輪就搶不到了（`js/lexicon.js`
   的 `form()` 是先來先贏）。這批是 `-f` 名詞的複數剛好等於 `-ve` 動詞的第三人稱：
   leaves／lives／halves／shelves／calves。兩邊都是對的字，所以不是「修好」而是
   「選一邊」—— 要嘛照詞頻決定，要嘛讓課程教過的字優先，要嘛查詢卡同時列出兩筆。
   **注意這不是規則推太寬造成的**：詞條本身永遠贏過任何變化形（`byWord` 先於 `byForm`），

...
1. **詞庫的例句**。商務標籤 284 字與第 1 級 1175 字都已補完（手寫層 645 筆、1290 句），剩下 5939 字連一句例句都沒有，仍是詞庫最弱的一環。補的方式一樣：搬進 `lexicon-core.js` 寫兩句，下一批是第 2 級的 766 字。第 2 級起有一批英式拼法（`centre`／`colour` 那種）與語氣詞，寫的時候直接說明它是哪一邊的拼法就好，不必當成另一個字。

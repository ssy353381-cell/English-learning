...
1. **起步偏好 A2：發音關的跳關測驗**。A1（`profile.track` + onboarding 問一題 + 設定頁可切換 + 發音關的 `planVocab` 配方）已完成，見「起步取向」。剩下的 A2：發音關卡加「跳關測驗」，通過就用既有的星數機制（`State.unit(id).s = 1`）解鎖下一關。使用者不是跳過，是證明不需要 —— 一樣不碰 `isUnlocked`。
